...
// ANSWER:
O(1)  //You don't need to iterate, just call on the 4th index.
...
// ANSWER:
O(1) //Constant Time
//You could console log one time using a comma to separate them
...
// Starting Code:
function squareEveryDigit(number) { //function called squareEveryDigit that takes a parameter of number
  newArr = ('' + number).split(''); // new array called newArr that converts the number into string and removes the ''s
  newArr2 = [];//new array called newArr2
  for (i = 0; i < newArr.length; i++) { //for loop to iterate through each element of the array
      let y = newArr[i] * newArr[i];//variable called y that equals to each element of the newArr array times itself
      newArr2.push(y);//adding the variable y to the array called newArr2 above
  }
  console.log(newArr2.join('')) //console logging the newArr2 and removing the '' using the .join method
}
